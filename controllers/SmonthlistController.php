...
$sql = "SELECT *
FROM 
(SELECT fyear,fmonth,nmonth,cid
FROM p_solary_ver s
LEFT JOIN tbmonth t ON t.mm=s.fmonth
WHERE cid='$user'
GROUP BY nmonth
UNION ALL
SELECT YEAR(mr.salarydate)+543 AS fyear, MONTH(mr.salarydate) AS fmonth,nmonth,cid
FROM p_money_report_sw mr 
LEFT JOIN tbmonth t ON t.mm=MONTH(mr.salarydate)
WHERE cid='$user'
GROUP BY fyear,fmonth) AS Tcc
GROUP BY  fyear,nmonth 
...
public function actionSearchsmonth() {
    $connection = Yii::$app->db;
    $ssn=$_GET['ssn'];
    $sql="SELECT CONCAT(nmonth,' ',fyear) AS tname,cid,fmonth,fyear
FROM(SELECT nmonth,fmonth,cid,fyear
FROM p_solary_ver s
LEFT JOIN tbmonth t ON t.mm=s.fmonth
WHERE md5(cid)='$ssn'
GROUP BY nmonth
#ORDER BY fyear,fmonth DESC
UNION ALL
SELECT nmonth,MONTH(mr.salarydate) AS fmonth,cid,YEAR(mr.salarydate)+543 as fyear
FROM p_money_report_sw  mr
LEFT JOIN tbmonth t ON t.mm=MONTH(mr.salarydate)
WHERE  md5(cid)='$ssn'
GROUP BY nmonth) AS  tcc
GROUP BY tname
ORDER BY fyear,fmonth DESC";
