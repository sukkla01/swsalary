...
//********************* header *******************************
$sqlc = "SELECT * 
FROM (SELECT s.fyear,t.nmonth,CONCAT(pname,fname,' ',lname) AS tname,s.cid,
...
WHERE md5(cid)='$user' AND fyear='$gyear' AND fmonth='$gmonth' LIMIT 1
UNION ALL
SELECT YEAR(salarydate)+543 AS fyear,t.nmonth,uf.name AS tname,mr.cid, 
				d.deptname as tdept,'' AS tbank,'' AS  bank_account
FROM p_money_report_sw mr
LEFT JOIN userinfo uf ON uf.ssn=mr.cid
LEFT JOIN dep d ON d.deptid=uf.DEFAULTDEPTID
LEFT JOIN tbmonth t ON t.mm=MONTH(salarydate)
WHERE md5(cid)='$user' AND YEAR(salarydate)='$gyear'-543  AND MONTH(salarydate)='$gmonth'
LIMIT 1) AS tvv
GROUP BY cid ";
